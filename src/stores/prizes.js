...
export const usePrizesStore = defineStore('prizes', () => {
  ...
  function importList(value) { if (!value?.name || !Array.isArray(value.prizes)) return false; const id = value.id && !lists.value[value.id] ? value.id : uid(); lists.value[id] = { id, name: value.name, prizes: value.prizes.map(p => ({ id: p.id || uid(), name: p.name, quality: p.quality || '', quantity: +p.quantity || 0, weight: Math.max(1, +p.weight || 1) })) }; currentId.value = id; save(); return true }
