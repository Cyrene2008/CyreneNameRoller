...
test('startup method is only shown while launch at sign-in is enabled', () => {
  assert.match(
    settingsView,
    /<Transition name="toggle-expand">\s*<div v-if="isTauri\(\) && settings\.autoStart" class="sub-setting">\s*<div v-if="isWindows" class="setting-row"><span class="setting-label">\{\{ lang === 'en' \? 'Startup method'/
  )
})
...
test('lack of administrator permission falls back to the traditional startup entry', () => {
  assert.match(settingsView, /isWindows\.value && result\?\.requiresElevation && value && mode === 'scheduled'/)
  assert.match(settingsView, /isWindows\.value && result\?\.requiresElevation && mode === 'scheduled'/)
