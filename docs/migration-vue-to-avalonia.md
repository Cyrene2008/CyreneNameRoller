...
C# 真实链路门禁（PerformanceGateTests）：Jint 抽取事务 < 100ms/次（全量测试负载下实测 ~26ms，隔离运行更低）、平衡选择 < 100ms/次、插件全栈加载 < 2s/次。
...
| ADR-13 | 插件 worker 协议引导层（activate/event/command/rpc 语义）抽为共享核心 `plugin-worker-protocol.js`，runtime.js 与 Jint 沙箱共用同一份模板 | 协议 SSOT，两端行为由构造一致 | 已定 |
| ADR-14 | 沙箱内 `self` 即 `globalThis`（与 Web Worker 语义一致）；插件执行以 plugin-dev 源码 bundle 为测试基准（发布混淆产物存在 onEvent 行为异常，属发布链路问题） | 实测：混淆 fixture worker 的 onEvent 在 Node 与 Jint 均零调用，干净 bundle 正常 | 已定 |
...
> 状态：**M4-3 已实现**（提交：M4-3 导航侧边栏）。落地阶段说明：已在 M4-3 完成主体实现；剩余增强（真滑动指示条、紧凑模式 Flyout 面板）排期 M6 后。实现时需对接 M2 声明树 `page.location = 'dock'` 的宿主渲染。
...
| C# 测试 | 55 全绿（Host 36 + App 19，含跨端一致性、契约对等、性能门禁） |
